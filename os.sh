...
# ══════════════════════════════════════════════════════════
#  1line — Cài đặt đầy đủ
# ══════════════════════════════════════════════════════════
1line() {
    apt update && apt upgrade -y
    termux-setup-storage 2>/dev/null || true
    pkg install zsh git figlet toilet ruby wget curl -y
    pkg install python nodejs clang ffmpeg -y
    pkg install boxes neofetch exa -y 2>/dev/null || true

    # Python packages
    pip install python-telegram-bot requests telebot rich 2>/dev/null || true
    pip install pystyle colorama yt_dlp flask telethon 2>/dev/null || true
    pip install python-telegram-bot==20.3 aiohttp pytz 2>/dev/null || true
    pip install beautifulsoup4 pywebview pyTelegramBotAPI 2>/dev/null || true
    pip install protobuf-decoder google-play-scraper python-cfonts 2>/dev/null || true
    pip install shortuuid aiofiles flask-cors 2>/dev/null || true
    pip install fastapi uvicorn python-multipart 2>/dev/null || true
    pip install qrcode protobuf PyJWT pillow 2>/dev/null || true
    pip install --upgrade pip setuptools wheel 2>/dev/null || true
    pip install "python-telegram-bot[job-queue]" 2>/dev/null || true

    # Node packages
    npm install -g discord.js 2>/dev/null || true
    ...
    if [ -d ~/Termux-os/.object ]; then
        ...
        [ -f .termux.properties ] && cp -r .termux.properties ~/.termux/termux.properties

        # Cursor style
        sed -i '/terminal-cursor-style/d' ~/.termux/termux.properties 2>/dev/null
        echo "terminal-cursor-style = underline" >> ~/.termux/termux.properties
    fi
...
(( ${+ZSH_HIGHLIGHT_STYLES} )) && {
    ...
    ZSH_HIGHLIGHT_STYLES[command-not-found]='fg=green,bold'
    ZSH_HIGHLIGHT_STYLES[default]='fg=green,bold'
}
...
_is_whitelisted() {
    case "$1" in
        lolcat|figlet|toilet|ls|ll|la|cd|pwd|clear|echo|cat|source|exit|kill|sleep|man|help|history|which|whereis|type|alias|unalias|export|unset|set|read|printf|test|true|false|python|python3|pip|pip3|node|npm|git|vim|nano|apt|pkg|bash|sh|zsh)
            return 0 ;;
        *) return 1 ;;
    esac
}
...
menu() {
    ...
    printf "\n%s${C}[${W}11${C}]${G} ⚡ Cài Smart Mode vào Shell ${Y}(Vĩnh viễn)" "$left_pad"
